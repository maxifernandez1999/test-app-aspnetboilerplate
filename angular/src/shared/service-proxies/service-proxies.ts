...
export class PlayerServiceProxy {
     ...
     * @return Success
     */
    get(id: number | undefined): Observable<PlayerDto> {
     ...
     * @return Success
     */
    getAll(sorting: string | undefined, skipCount: number | undefined, maxResultCount: number | undefined): Observable<PlayerDtoPagedResultDto> {
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            ...
            if (response_ instanceof HttpResponseBase) {
                try {
                    ...
                    return _observableThrow(e) as any as Observable<PlayerDtoPagedResultDto>;
                }
            } else
                return _observableThrow(response_) as any as Observable<PlayerDtoPagedResultDto>;
        }));
    }

    protected processGetAll(response: HttpResponseBase): Observable<PlayerDtoPagedResultDto> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
        ...
        if (status === 200) {
            ...
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = PlayerDtoPagedResultDto.fromJS(resultData200);
            return _observableOf(result200);
            }));
        } else if (status !== 200 && status !== 204) {
            return blobToText(responseBlob).pipe(_observableMergeMap((_responseText: string) => {
     ...
     * @return Success
     */
    create(body: PlayerDto | undefined): Observable<PlayerDto> {
     ...
     * @return Success
     */
    update(body: PlayerDto | undefined): Observable<PlayerDto> {
...
export class PlayerDtoPagedResultDto implements IPlayerDtoPagedResultDto {
    items: PlayerDto[] | undefined;
    totalCount: number;

    constructor(data?: IPlayerDtoPagedResultDto) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    static fromJS(data: any): PlayerDtoPagedResultDto {
        data = typeof data === 'object' ? data : {};
        let result = new PlayerDtoPagedResultDto();
        result.init(data);
        return result;
    }
...
export interface IPlayerDtoPagedResultDto {
    items: PlayerDto[] | undefined;
    totalCount: number;
}
